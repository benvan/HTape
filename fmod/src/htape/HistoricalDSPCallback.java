...
*/
class HistoricalDSPCallback implements FMOD_DSP_READCALLBACK {

    int pos = 0;
    StereoHistoryBuffer hist;
    IFilter filter;

    HistoricalDSPCallback(int initialSize, IFilter filter) {
        this(filter);
        hist.resize(initialSize);
    }

    HistoricalDSPCallback(IFilter filter) {
        hist = new StereoHistoryBuffer();
        this.filter = filter;
    }

    HistoricalDSPCallback() {
