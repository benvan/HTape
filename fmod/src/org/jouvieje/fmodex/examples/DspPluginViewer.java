		...
		if(timer == null) {
			timer = new Timer(10, new ActionListener(){
				public void actionPerformed(ActionEvent e) {
					if(!initialized) if(initialize()) initialized = true;
					else exit(0);
...
//		this.getTimer().start();
	...
	protected void addPlugins() {
		...
		if(choice == JFileChooser.APPROVE_OPTION) {
			...
			for(int i = 0; i < files.length; i++) {
				FMOD_RESULT result = addPlugin(files[i].getPath());
				errorCheck(result);
	...
	private IntBuffer buff = newIntBuffer(1);

	FMOD_RESULT addPlugin(String pluginPath) {
		FMOD_PLUGINTYPE[] plugintype = new FMOD_PLUGINTYPE[1];
		FMOD_RESULT result;

		result = gSystem.loadPlugin(pluginPath, buff, 0);
		if(result != FMOD_OK) {
			return result;
		}
		int handle = buff.get(0);

		result = gSystem.getPluginInfo(handle, plugintype, null, 0, null);
