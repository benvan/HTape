...
import static org.jouvieje.fmodex.defines.FMOD_INITFLAGS.FMOD_INIT_GEOMETRY_USECLOSEST;
...
public class Geometry_ implements GLEventListener, WindowListener, KeyListener, MouseListener, MouseMotionListener {
	...
	Object3D[] objects = new Object3D[]{new Object3D(-11.0f, 1.0f, 0.0f, 1.0f, 0, new Channel()),
	...
	public void init(GLAutoDrawable GLAutoDrawable) {
		...
		out.printf("==================================================================\n");
		out.printf("Geometry example.  Copyright (c) Firelight Technologies 2004-2009.\n");
		out.printf("==================================================================\n\n");
		...
		result = fmodSystem.getNumDrivers(buffer.asIntBuffer());
		errorCheck(result);
		...
		if(numdrivers == 0) {
			...
			{
				result = fmodSystem.setSoftwareFormat(48000, FMOD_SOUND_FORMAT_PCMFLOAT, 0, 0,
...
//        errorCheck(result);
		...
		result = fmodSystem.init(100, FMOD_INIT_3D_RIGHTHANDED | FMOD_INIT_SOFTWARE_OCCLUSION | FMOD_INIT_SOFTWARE_HRTF
				| FMOD_INIT_GEOMETRY_USECLOSEST, null);
		if(result == FMOD_ERR_OUTPUT_CREATEBUFFER) /* Ok, the speaker mode selected isn't supported by this soundcard.  Switch it back to stereo... */
		{
			...
			result = fmodSystem.init(100, FMOD_INIT_3D_RIGHTHANDED | FMOD_INIT_SOFTWARE_OCCLUSION
					| FMOD_INIT_SOFTWARE_HRTF | FMOD_INIT_GEOMETRY_USECLOSEST, null);/* ... and re-init. */
			errorCheck(result);
		}
	...
	void timerFunc() {
		...
		if(false) {
			...
			out.printf("chans %d : cpu : dsp = %.02f geometry = %.02f stream = %.02f update = %.02f total = %.02f\n",
					chansplaying.get(0), dsp.get(0), geometry.get(0), stream.get(0), update.get(0), total.get(0));
		}
	}
	...
	void doGeometryMovement() {
		...
		FMOD_VECTOR up = FMOD_VECTOR.allocate(0.0f, 1.0f, 0.0f);
		FMOD_VECTOR forward = FMOD_VECTOR.allocate((float)sin(accumulatedTime * 0.5f), 0.0f,
				(float)cos(accumulatedTime * 0.5f));
		...
		if(pos.getY() < 0.0f) pos.setY(0);
		if(pos.getY() > 2.0f) pos.setY(2.0f);
		pos.setZ(11.5f);
	...
	FMOD_VECTOR lastVel = FMOD_VECTOR.allocate(0.0f, 0.0f, 0.0f);
	boolean bFirst = true;
	...
	void doListenerMovement() {
		...
		if(moveForward) forward += MOVEMENT_SPEED * INTERFACE_UPDATETIME / 17.0f; //INTERFACE_UPDATETIME in FMOD Ex is 17
		if(moveBackward) forward -= MOVEMENT_SPEED * INTERFACE_UPDATETIME / 17.0f;

		float right = 0.0f;
		if(moveLeft) right -= MOVEMENT_SPEED * INTERFACE_UPDATETIME / 17.0f;
		if(moveRight) right += MOVEMENT_SPEED * INTERFACE_UPDATETIME / 17.0f;

		float up = 0.0f;
		if(moveUp) up += MOVEMENT_SPEED * INTERFACE_UPDATETIME / 17.0f;
		if(moveDown) up -= MOVEMENT_SPEED * INTERFACE_UPDATETIME / 17.0f;
		...
		// Update listener
		{
			FMOD_VECTOR listenerVector = FMOD_VECTOR.allocate();
			listenerVector.setX(xListenerPos);
			listenerVector.setY(yListenerPos);
			listenerVector.setZ(zListenerPos);

			FMOD_VECTOR forwardVec = FMOD_VECTOR.allocate();
			FMOD_VECTOR upVec = FMOD_VECTOR.allocate();
			FMOD_VECTOR vel = FMOD_VECTOR.allocate();
